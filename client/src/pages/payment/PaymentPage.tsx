...
const PaymentPage = () => {
  ...
  const roomDetails = roomsData.map((r) => ({
    number: r.roomNumber,
    noOfExtraGuests: r.extraGuests,
    noOfExtraBeds: r.extraBeds,
    people: {
      adult: r.adult,
      children: r.children,
    },
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full p-10 sm:p-14">
        ...
        <section className="mb-12 mt-10">
          <h2 className="text-2xl font-semibold mb-6 text-indigo-700">Selected Rooms & Extras</h2>
          {roomsData.length === 0 ? (
            <p className="text-gray-500 text-lg">No rooms selected.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
              {roomsData.map((r) => (
                <div key={r.roomNumber} className="border border-indigo-300 rounded-xl p-5 bg-indigo-50 shadow-sm hover:shadow-md transition">
                  <p className="text-lg font-semibold text-indigo-900 mb-2">Room #{r.roomNumber}</p>
                  <p className="text-gray-700">Adults: <span className="font-medium">{r.adult}</span></p>
                  <p className="text-gray-700">Children: <span className="font-medium">{r.children}</span></p>
                  <p className="text-gray-700">Extra Guests: <span className="font-medium">{r.extraGuests}</span></p>
                  <p className="text-gray-700">Extra Beds: <span className="font-medium">{r.extraBeds}</span></p>
                </div>
              ))}
            </div>
          )}
